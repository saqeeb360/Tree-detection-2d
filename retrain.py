...
checkpoint_path = os.path.join(MODEL_DIR, BASE_MODEL)
checkpoint = torch.load(checkpoint_path, map_location=device)

# Initialize the model
model =  torchvision.models.detection.maskrcnn_resnet50_fpn(pretrained=False)  # Pretrained weights are not needed
...
# Assuming optimizer was saved in the checkpoint (if not, initialize a new one)
...
def main():
    ...
    for epoch in range(TOTAL_EPOCH+1):
        ...
        if val_epoch_loss < best_val_loss or epoch % 5 == 0:
            ...
            torch.save(checkpoint, os.path.join(MODEL_DIR,f"{TRAIN_MODEL} {epoch}.pth.tar"))
